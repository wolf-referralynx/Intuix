...
TextEditor::TextEditor()
	...
	SetLanguageDefinition(LanguageDefinition::Javascript());
...
void TextEditor::ToggleComment() {
    ...
    // Determine the range of lines to comment/uncomment
    ...
    if (startLine == endLine && mState.mSelectionStart == mState.mSelectionEnd) {
        // No selection, toggle the current line
        ToggleCommentLine(startLine);
    } else {
        // Multiple lines selected, toggle all within range
        for (int line = startLine; line <= endLine; ++line) {
            ToggleCommentLine(line);
        }
    }
...
void TextEditor::ToggleCommentLine(int line) {
    if (line < 0 || line >= (int)mLines.size()) return;

    auto& currentLine = mLines[line];
    const std::string commentPrefix = "//";

    if (!currentLine.empty() && currentLine[0].mChar == '/' && currentLine.size() > 1 && currentLine[1].mChar == '/') {
        // Line is commented, remove the comment prefix
        currentLine.erase(currentLine.begin(), currentLine.begin() + 2);
    } else {
        // Line is not commented, add the comment prefix
        currentLine.insert(currentLine.begin(), Glyph('/', PaletteIndex::Comment));
        currentLine.insert(currentLine.begin(), Glyph('/', PaletteIndex::Comment));
    }
}
...
void TextEditor::HandleKeyboardInputs() {
	...
	if (ImGui::IsWindowFocused()){
		...
		else if(ctrl && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Slash))){
			if (ImGui::IsWindowFocused()) {
				if (!IsReadOnly() && ctrl && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Slash))) {
					ToggleComment();
					Colorize();
				}
    		}
...
const TextEditor::Palette & TextEditor::GetDarkPalette() {
    const static Palette p = { {
        0xffd4d4d4, // Default (light gray for regular text)
        0xff569cd6, // Keyword (blue)
		//0xff0000ff, // Keyword (purple) for testing
        0xffb5cea8, // Number (light green)
        0xffce9178, // String (salmon)
        0xffce9178, // Char literal (salmon, matching strings)
        0xffd4d4d4, // Punctuation (light gray)
        0xffc586c0, // Preprocessor (purple)
        0xff9cdcfe, // Identifier (light blue)
        0xff4ec9b0, // Known identifier (teal)
        0xffc586c0, // Preprocessor identifier (purple, matching preprocessor)
        0xff6a9955, // Comment (single line, green)
        0xff6a9955, // Comment (multi-line, green)
        0xff1e1e1e, // Background (dark gray)
        0xffffffff, // Cursor (white)
        0x80ffffff, // Selection (translucent white)
        0xffff5555, // ErrorMarker (red)
        0xffff0000, // Breakpoint (bright red)
        0xff858585, // Line number (dim gray)
        0x40ffffff, // Current line fill (translucent white)
        0x40cccccc, // Current line fill (inactive, translucent light gray)
        0x40808080, // Current line edge (gray)
    } };
    return p;
}
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::Javascript() {
    ...
    if (!initialized) {
        ...
        // Define JavaScript Built-in Identifiers
        static const char* const identifiers[] = {
            "Array", "Date", "eval", "function", "hasOwnProperty", "Infinity", "isFinite", "isNaN", "isPrototypeOf", "length",
            "Math", "NaN", "name", "Number", "Object", "prototype", "String", "toString", "undefined", "valueOf", "Promise",
            "Map", "Set", "WeakMap", "WeakSet", "console", "JSON"
        };
        for (const auto& identifier : identifiers) {
            Identifier id;
            id.mDeclaration = "Built-in object";
            langDef.mIdentifiers[identifier] = id;
        }

        // Define Token Regex Strings
        langDef.mTokenRegexStrings.push_back({ "//.*", PaletteIndex::Comment });                             // Single-line comments
        langDef.mTokenRegexStrings.push_back({ "/\\*.*?\\*/", PaletteIndex::Comment });                      // Multi-line comments
        langDef.mTokenRegexStrings.push_back({ R"(".*?")", PaletteIndex::String });                          // Double-quoted strings
        langDef.mTokenRegexStrings.push_back({ R"('.*?')", PaletteIndex::String });                          // Single-quoted strings
        langDef.mTokenRegexStrings.push_back({ R"(`.*?`)", PaletteIndex::String });                          // Template literals
        langDef.mTokenRegexStrings.push_back({ R"([+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?)", PaletteIndex::Number }); // Numbers
        langDef.mTokenRegexStrings.push_back({ R"([a-zA-Z_$][a-zA-Z0-9_$]*)", PaletteIndex::Identifier });   // Identifiers
        langDef.mTokenRegexStrings.push_back({ R"([\[\]\{\}\!\%\^\&\*\(\)\-\+\=\~\|\<\>\?\/\;\,\.\:])", PaletteIndex::Punctuation }); // Punctuation
